...
// will be slightly crappier
	...
	function parseSource(source, command) {
		...
		for (let i = 0; i < lines.length; ++i) {
			const line = lines[i];
			const parts = /^\s*#\s*(\w+)\s+(.+)\s*$/.exec(line);
			if (parts) {
				switch (parts[1]) {
					case 'line':
						const lineNumberInfo = /(\d+)(\s+\d+)?/.exec(parts[2]);
						if (lineNumberInfo) {
						...
						break
					case 'define':
						const nameInfo = /SHADER_NAME(_B64)?\s+(.*)$/.exec(parts[2]);
						if (nameInfo) {
	...
	function annotateFiles(files, errors) {
		errors.forEach(function (error) {
			...
			if (file) {
				const line = file.index[error.line];
				if (line) {
	...
	function checkLinkError(gl, program, fragShader, vertShader, command) {
		if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
			...
			const header = 'Error linking program with vertex shader, "' +
...
// zero pool for initial zero data
	...
	const wrapLimits = function (gl, extensions) {
		...
		if (!isIE) {
			...
			const data = pool.allocType(GL_UNSIGNED_BYTE$1, 36);
